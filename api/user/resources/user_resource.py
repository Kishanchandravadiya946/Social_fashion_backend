...
class UserResource:
    ...
    def create_user():
        ...
        existing_user = SiteUser.query.filter_by(
            email_address=data.get('email_address')).first()
        if existing_user:
            if existing_user.verify_user:
                return jsonify({'error': 'Email already exists'}), 400
            else:
                ShoppingCart.query.filter_by(user_id=existing_user.id).delete()
        ...
        try:
            ...
            redis_client.setex(
                f"verify:{data['email_address']}", 600, verification_code)
            UserResource.send_verification_email(
                data['email_address'], verification_code)

            return jsonify({'message': 'verify with your email!', 'user_id': new_user.id}), 201
        except Exception as e:
            db.session.rollback()
            print(str(e))
    ...
    def verify_user():
        ...
        user = SiteUser.query.filter_by(email_address=email).first()
        if user:
    ...
    def address_create():
        try:
            ...
            # print("x")
            required_fields = ["unit_number", "street_number", "address_line1",
                               "city", "region", "postal_code", "country_id", "user_id"]
            print("a")
            missing_fields = [
    ...
    def address_fetch():
        try:
            ...
            default_add = next(
                (ua.address_id for ua in user_addresses if ua.is_default), None)
            # print(default_add);
            # print("b")
            addresses = db.session.query(
                ...
                Address.region,
                Address.postal_code,
                # Fetch country name instead of country_id
    ...
    def address_set(address_id):
        ...
        try:
            ...
            UserAddress.query.filter_by(
                user_id=user.id, is_default=True).update({"is_default": False})

        # Set the new default address
            new_default_address = UserAddress.query.filter_by(
                user_id=user.id, address_id=address_id).first()
    ...
    def address_delete(address_id):
        try:
            ...
            UserAddress.query.filter_by(
                user_id=user.id, address_id=address_id).delete()
